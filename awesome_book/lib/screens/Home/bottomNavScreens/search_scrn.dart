...
// //search api
//   // Future<void> SearchForpeople() async {
//   //   print("Fetching posts for u ");
//   //   if (isLoading) return;
//   //   setState(() => isLoading = true);
//   // }
...
import 'package:awesome_book/models/posts_model.dart';
import 'package:awesome_book/screens/Home/bottomNavScreens/search2_scrn.dart';
import 'package:awesome_book/screens/Home/bottomNavScreens/searchPeople_data.dart';
import 'package:awesome_book/utils/post_fullscreen.dart';
import 'package:awesome_book/utils/reels_fullScreen.dart';
import 'package:awesome_book/widgets/mytext.dart';
import 'package:flutter/material.dart';
...
class _Search_scrnState extends State<Search_scrn> {
  ...
  /// 🔍 NEW FUNCTION: Search for People
...
//  TextEditingController _searchController = TextEditingController();
  ...
  Future<void> searchPeople(String query) async {
    ...
    try {
      Uri url = Uri.parse(glb.API.search);
      var res = await http.post(url, body: {"search_query": query});

      print("🔍 Search API called with query: $query");
      ...
      if (res.statusCode == 200) {
        ...
        if (body is List && body.isNotEmpty) {
          setState(() {
            searchResults = List<Map<String, dynamic>>.from(body);
          });
        } else {
          setState(() {
            searchResults.clear();
                ...
                return ListTile(
                  leading: CircleAvatar(
                    backgroundImage: NetworkImage(
                      glb.API.baseURL + (user['profile_img'] ?? ''),
                    ),
                  ),
                  ...
                  onTap: () {
                    ...
                    // Optional: Navigate to profile page if needed
                  },
                );
              },
            )
